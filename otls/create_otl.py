...
def space_camel_case(str):
    return re.sub(r'((?<=[a-z])[A-Z]|(?<!\A)[A-Z](?=[a-z]))', r' \1', str)


def build_aovs(folder, aovs, parmType='toggle'):
    for aov in aovs:
        ...
        if parmType == 'toggle':
            ...
            folder.addParmTemplate(hou.StringParmTemplate('aov{}'.format(name), label, 1))


def setup_custom_aovs(dist, name):
    disable = '{{aov{}CustomDisable_# == 1}}'.format(name)
    folder = hou.FolderParmTemplate('{}Custom'.format(name.lower()), 'Custom AOVs',
                                    folder_type=hou.folderType.Collapsible)
    custom = hou.FolderParmTemplate('{}CustomAOVs'.format(name.lower()), 'Extra Image Planes',
                                    folder_type=hou.folderType.MultiparmBlock)
    custom.addParmTemplate(hou.ToggleParmTemplate('aov{}CustomDisable_#'.format(name), 'Disable AOV'))
    custom.addParmTemplate(hou.StringParmTemplate('aov{}CustomName_#'.format(name), 'Name', 1, disable_when=disable))
    custom.addParmTemplate(hou.MenuParmTemplate('aov{}CustomSource_#'.format(name), 'Source',
                                                ('color', 'float', 'integer', 'vector', 'normal', 'point'),
                                                ('Color', 'Float', 'Integer', 'Vector', 'Normal', 'Point'),
                                                join_with_next=True, disable_when=disable))
    custom.addParmTemplate(hou.StringParmTemplate('aov{}CustomLPE_#'.format(name), 'LPE', 1, is_label_hidden=True,
...
for key, value in hou.vopNodeTypeCategory().nodeTypes().items():
    ...
    if 'pxr' in name and value.hasSectionData('Tools.shelf'):
        if 'Integrator' in ET.fromstring(value.sectionData('Tools.shelf')).find('tool').findtext('toolSubmenu'):
            inte = integrators.createNode(name, name)
            integrator_list.append((name, space_camel_case(value.description().replace('Pxr', ''))))
...
# Create HDA
#
hda = hou.Node.createDigitalAsset(hda, 'sgtk_ris', r"D:\Developer\Pipeline\tk-houdini-renderman\otls\sgtk_ris.otl",
                                  version='25.2', ignore_external_references=True)
hda.type().setDefaultColor(hou.Color(0, 0.2, 0.3))

hda_def = hda.type().definition()
hda_options = hda_def.options()

python_module = open(r'D:\Developer\Pipeline\tk-houdini-renderman\otls\PythonModule.py', "r")
...
hda_parms.append(hou.ButtonParmTemplate("executeFarm", "Submit to Farm", join_with_next=True,
                                        script_callback='hou.phm().render(True)',
                                        script_callback_language=hou.scriptLanguage.Python))
hda_parms.append(hou.ButtonParmTemplate("executeLocal", "Render to Disk", join_with_next=True,
                                        script_callback='hou.phm().render(False)',
                                        script_callback_language=hou.scriptLanguage.Python))
hda_parms.append(hou.ButtonParmTemplate("copyPathToClipboard", "Copy path to clipboard", join_with_next=True,
                                        script_callback='hou.phm().copy_to_clipboard()',
                                        script_callback_language=hou.scriptLanguage.Python))
hda_parms.append(hou.ButtonParmTemplate("openStats", "Open render statistics",
...
for p in ('trange', 'f', 'camera', 'override_camerares', 'res_fraction', 'res_override', 'res_overrideMenu',
          'aspect_override'):
    conditional = None
    if p == 'f':
...
rendering.addParmTemplate(hou.MenuParmTemplate('integrator', 'Integrator', tuple(it_names), tuple(it_labels),
                                               default_value=it_names.index('pxrpathtracer')))
rman.parm('shop_integratorpath').setExpression('"../{}/" + chs("../integrator")'.format(integrators.name()))

for p in ('ri_hider_type', 'ri_dof', 'allowmotionblur', 'ri_hider_samplemotion', 'xform_motionsamples',
          'geo_motionsamples', 'shutteroffset'):
    reference_parm(rman, rendering, p)
    if p == 'ri_hider_samplemotion':
        tmp = rendering.parmTemplates()
        tmp[-1].setMenuLabels(('2D Motion Blur (Motion Vectors)', '3D Motion Blur (Render)'))
...
raydepth = hou.FolderParmTemplate('raydepth2', 'Default Ray Depth', folder_type=hou.folderType.Collapsible)
...
for (name, label) in integrator_list:
    ...
    intf.setTabConditional(hou.parmCondType.HideWhen, '{{ integrator != {} }}'.format(name))
...
aovs_folder.addParmTemplate(hou.ButtonParmTemplate('setupAOVs', 'Setup AOV\'s',
...
reference_parm(denoise, aovs_folder, 'mode', (hou.parmCondType.HideWhen, '{ denoise == 0 }'))
...
aovs_shading_lobes = hou.FolderParmTemplate('shadingLobes', 'Lobes', folder_type=hou.folderType.Simple)
shading_lobes_aovs = (
...
aovs_lighting_shadow = hou.FolderParmTemplate('lightingShadow', 'Shadow', folder_type=hou.folderType.Simple)
shadow_msg = hou.LabelParmTemplate('shadowInfo', 'Info', column_labels=('Enable the Holdout tag for an object to show '
                                                                        'up in the shadow aovs',))
...
aovs_lighting_groups = hou.FolderParmTemplate('lightingGroups', 'Light Groups', folder_type=hou.folderType.Simple)
lg_aovs = list(map(lambda x: "Cat" + chr(x), range(65, 75)))
lg_aovs.insert(0, 'EyeReflection')
for lg in lg_aovs:
    aovs_lighting_groups.addParmTemplate(hou.ToggleParmTemplate('aovLGUse{}'.format(lg), '', is_label_hidden=True,
                                                                join_with_next=True))
    aovs_lighting_groups.addParmTemplate(hou.StringParmTemplate('aovLG{}'.format(lg), lg, 1, default_value=(lg,),
...
aovs_tee = hou.FolderParmTemplate('tees', 'Shading AOVs (Tee)', folder_type=hou.folderType.MultiparmBlock)
aovs_tee.addParmTemplate(hou.MenuParmTemplate('teeType_#', '',
                                              ('color', 'float', 'integer', 'vector', 'normal', 'point'),
                                              ('Color', 'Float', 'Integer', 'Vector', 'Normal', 'Point'),
                                              join_with_next=True, is_label_hidden=True))
aovs_tee.addParmTemplate(hou.StringParmTemplate('teeName_#', '    Name', 1))
aovs_utility.addParmTemplate(aovs_tee)
tee_msg = hou.LabelParmTemplate('teeInfo', 'Info', column_labels=('Use a PxrTee node in your shader to export a '
                ...
                for p2 in p.parmTemplates():
                    if p2.label() == 'Metadata':
                        ...
                        for p3 in temp:
                            ...
                            if cond:
                                p3.setConditional(hou.parmCondType.HideWhen, cond.replace('_#_#', "_#"))
