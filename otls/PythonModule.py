...
def setupAOVs(show_notif=True):
    ...
    crypto = list(filter(lambda parm: 'Crypto' in parm.name() and parm.eval() == 1, aovs))
    ...
    lightgroups = list(
        map(make_lightgroups, list(filter(lambda parm: 'LGUse' in parm.name() and parm.eval() == 1, aovs)))
    )

    tee_count = hda.evalParm('tees')

    shading = hda.parmsInFolder(('AOVs', 'Shading'))
    shading = list(filter(lambda parm: 'aov' in parm.name() and parm.eval() == 1, shading))

    lighting = hda.parmsInFolder(('AOVs', 'Lighting'))
    lighting = list(filter(lambda parm: 'aov' in parm.name() and parm.eval() == 1, lighting))

    utility = hda.parmsInFolder(('AOVs', 'Utility'))
    utility = list(filter(lambda parm: 'aov' in parm.name() and parm.eval() == 1, utility))
    ...
    # Statistiscs
    rman.parm('ri_statistics_xmlfilename').set(app.get_output_path(hda, 'stats')[:-3] + 'xml')
    ...
    if len(shading):
        ...
        rman.parm('ri_quickaov_directDiffuseUnoccluded_' + str(i)).set('DiffuseU' in shading)
        rman.parm('ri_quickaov_indirectDiffuseUnoccluded_' + str(i)).set('DiffuseU' in shading)
        ...
        rman.parm('ri_quickaov_directSpecularUnoccluded_' + str(i)).set('SpecularU' in shading)
        rman.parm('ri_quickaov_indirectSpecularUnoccluded_' + str(i)).set('SpecularU' in shading)
        rman.parm('ri_quickaov_subsurface_' + str(i)).set('Subsurface' in shading)

        rman.parm('ri_quickaov_directDiffuseLobe_' + str(i)).set('LobeDiffuse' in shading)
        rman.parm('ri_quickaov_indirectDiffuseLobe_' + str(i)).set('LobeDiffuse' in shading)
        rman.parm('ri_quickaov_directSpecularPrimaryLobe_' + str(i)).set('LobeSpecularPrimary' in shading)
        rman.parm('ri_quickaov_indirectSpecularPrimaryLobe_' + str(i)).set('LobeSpecularPrimary' in shading)
        rman.parm('ri_quickaov_directSpecularRoughLobe_' + str(i)).set('LobeSpecularRough' in shading)
        rman.parm('ri_quickaov_indirectSpecularRoughLobe_' + str(i)).set('LobeSpecularRough' in shading)
        rman.parm('ri_quickaov_directSpecularClearcoatLobe_' + str(i)).set('LobeSpecularClearcoat' in shading)
        rman.parm('ri_quickaov_indirectSpecularClearcoatLobe_' + str(i)).set('LobeSpecularClearcoat' in shading)
        rman.parm('ri_quickaov_directSpecularIridescenceLobe_' + str(i)).set('LobeSpecularIridescence' in shading)
        rman.parm('ri_quickaov_indirectSpecularIridescenceLobe_' + str(i)).set('LobeSpecularIridescence' in shading)
        rman.parm('ri_quickaov_directSpecularFuzzLobe_' + str(i)).set('LobeSpecularFuzz' in shading)
        rman.parm('ri_quickaov_indirectSpecularFuzzLobe_' + str(i)).set('LobeSpecularFuzz' in shading)
        rman.parm('ri_quickaov_directSpecularGlassLobe_' + str(i)).set('LobeSpecularGlass' in shading)
        rman.parm('ri_quickaov_indirectSpecularGlassLobe_' + str(i)).set('LobeSpecularGlass' in shading)
        rman.parm('ri_quickaov_subsurfaceLobe_' + str(i)).set('LobeSubsurface' in shading)
        rman.parm('ri_quickaov_transmissiveSingleScatterLobe_' + str(i)).set(
            'LobeTransmissiveSingleScatter' in shading)
        rman.parm('ri_quickaov_transmissiveGlassLobe_' + str(i)).set('LobeTransmissiveGlass' in shading)
    ...
    if len(lighting) or len(lightgroups):
        ...
        rman.parm('ri_quickaov_unoccluded_' + str(i)).set('ShadowUnoccluded' in lighting)
        ...
        for j, group in enumerate(lightgroups):
            ...
            rman.parm('ri_aovsource_' + str(i) + '_' + str(j)).set(f'color lpe:C.*<L.\'{group[1]}\'>')
    ...
    if len(utility) or tee_count:
        ...
        for j in range(tee_count):
            rman.parm('ri_aovtype_' + str(i) + '_' + str(j)).set(hda.parm('teeType_' + str(j + 1)).evalAsString())
            rman.parm('ri_aovvariable_' + str(i) + '_' + str(j)).set(hda.evalParm('teeName_' + str(j + 1)))
    ...
    for i, c in enumerate(crypto):
        ...
        hda.parm('./aovs/' + name + '/filename').set(
            app.get_output_path(hda, name)
        )
    ...
    md_parms = list(filter(lambda parm: 'exr_metadata' in parm.name() and parm.name() != 'ri_exr_metadata',
                           hda.parms()))

    for f in range(file_count):
        rman.parm('ri_exr_metadata_{}'.format(f)).set(md_count_node + md_count_external + (len(md_config) > 0))
        ...
        if len(lightgroups):
            rman.parm('ri_exr_metadata_key_{}_{}'.format(f, md_count_node - 1)).set('rmd_lightgroups')
            rman.parm('ri_exr_metadata_type_{}_{}'.format(f, md_count_node - 1)).set('string')
            rman.parm('ri_exr_metadata_string_{}_{}_'.format(f, md_count_node - 1)).set(md_lg)

        for i in range(md_count_external):
            item = md_config[i]
            rman.parm('ri_exr_metadata_key_{}_{}'.format(f, md_count_node + i)).set('rmd_{}'.format(item.get('key')))
            rman.parm('ri_exr_metadata_type_{}_{}'.format(f, md_count_node + i)).set(item.get('type'))
            rman.parm('ri_exr_metadata_{}_{}_{}_'.format(item.get('type'), f, md_count_node + i)).setExpression(
                item.get('expression'))

        rman.parm('ri_exr_metadata_key_{}_{}'.format(f, md_count_node + md_count_external)).set('rmd_groups')
        rman.parm('ri_exr_metadata_type_{}_{}'.format(f, md_count_node + md_count_external)).set('string')
        rman.parm('ri_exr_metadata_string_{}_{}_'.format(f, md_count_node + md_count_external)).set(md_config_groups)
...
def get_render_paths():
    ...
    lightgroups = list(filter(lambda parm: 'LGUse' in parm.name() and parm.eval() == 1, aovs))
...
def __set_expression(node, source_parm, dist_parm):
    ...
    node.parm(dist_parm).setExpression('{}("{}{}")'.format(parm_type, "../", source_parm))
